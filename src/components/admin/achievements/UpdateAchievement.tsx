...
const UpdateAchievement = ({ location, params }: { location: any; params: any }) => {
    ...
    useEffect(() => {
        const fetchAchievement = async () => {
            if (!slug) return;
            try {
                const response = await axios.get(`/api/achievements/${slug}`);
                const achievement: any = response.data;


                achievement.content_items.sort((a: any, b: any) => a.order - b.order);
                ...
                const parsedContentItems = achievement.content_items.map((item: any) => {
                    if (item.type === "list") {
                ...
                if (!["en", "fr"].includes(paramLang || "")) {
                    setLanguage(achievement.title_en ? "en" : "fr");
                }
            } catch (error) {
                console.error("Error fetching blog achievement:", error);
            }
        };
    ...
    const handleLanguageChange = (e: React.ChangeAchievement<HTMLSelectElement>) => {
        setLanguage(e.target.value);
    };

    const handleChange = (e: React.ChangeAchievement<HTMLInputElement | HTMLTextAreaElement>) => {
        const { name, value } = e.target;
        setFormData((prev) => ({ ...prev, [name]: value }));
    };

    const handleImageChange = (e: React.ChangeAchievement<HTMLInputElement>) => {
        if (e.target.files?.[0]) {
            setImage(e.target.files[0]);
        }
    };

    const handleItemContentChange = (itemIndex: number, achievement: any) => {
        ...
        if (item.type === "image" || item.type === "pdf") {
            const file = achievement.target.files?.[0];
            if (file) {
                item.file = file;
            ...
            // text, title, etc.
            item.content = achievement.target.value;
            if (language === "en") {
                setEnglishItems([...updatedItems]);
            } else {
                setFrenshItems([...updatedItems]);
    ...
    const handleSubmit = async (e: React.FormAchievement) => {
        e.preventDefault();
        ...
        try {
            const response = await axios.post(`/api/achievements/${slug}`, formDataToSend, {
