...
export default function ManageAchievement({ params }: { params: any }) {
    ...
    const handleAddTheme = (e: React.FormEvent) => {
        ...
        if (achievement.themes.length >= 4) {
            Swal.fire("Error", "An achievement can have a maximum of 4 themes.", "error");
            return;
        }

        axios
            .post("/api/add-achievement-theme", {
    ...
    const deleteTheme = (e: any, item: any) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios
                    .delete(`/api/achievement/${achievement.id}/theme/${item.id}`)
                    .then((res) => {
                        Swal.fire("Deleted!", "The achievement has been removed from this achievement.", "success");
                        setThemeId("");
                        getAchievement();
                    })
                    .catch((err) => {
        ...
        <div className="container mx-auto p-6">
            {loading ? (
                ...
                <div className="bg-white shadow-md rounded-lg p-6">
                    {/* Header */}
                    <div className="mb-6">
                        <h1 className="text-4xl font-extrabold mb-2">Manage Achievments</h1>
                        <p className="text-lg text-gray-600">
                            Titre: <span className="font-semibold">{achievement.title_en || achievement.title_fr}</span>
                        </p>
                        <img
                        ...
                        />
                        <p className="text-lg text-gray-600">
                            Description: <span className="font-semibold">{achievement.description_en || achievement.description_fr}</span>
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <form onSubmit={handleAddTheme} className="flex flex-col sm:flex-row items-center gap-4">
                            ...
                            >
                                ...
                                {achievement.themesNotSelected?.map((theme: any) => (
