...
const EditPost = ({ location, params }: { location: any, params: any }) => {
  ...
  const onItemsSortEnd = ({ oldIndex, newIndex }: { oldIndex: number, newIndex: number }) => {
    ...
    const updatedItems = arrayMove(language == "en" ? [...englishItems] : [...frenshItems], oldIndex, newIndex).map((item, idx) => { return { ...item, order: idx } });
  ...
  return (
    <div className="h-[calc(100vh-80px)] flex flex-col p-4">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col grow">
        ...
        {((language == "en" && englishItems.length) || (language == "fr" && frenshItems.length)) ? <ItemsList
          useDragHandle
