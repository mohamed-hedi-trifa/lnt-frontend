...
export default function ResearchKnowledge({ location, params }: { location: any; params: any }) {
    ...
    function getResearchKnowledge(slug) {
        axios
            .get(`/api/research-knowledge/${slug}`)
            .then((res) => {
    ...
    useEffect(() => {
        if (params.slug) {
            ...
            getResearchKnowledge(params.slug);
        }
    ...
    const deleteItem = (e: any, item: any) => {
        ...
        Swal.fire({
            title: "Delete Research & Knowledge",
            text: `Are you sure to delete ${item.title || item.title} ?`,
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                axios
                    .delete(`/api/research-knowledge/${item.id}`)
                    .then((res) => {
                    ...
                    <div className="p-5">
                        {itemsList?.length > 0 ? (
                            ...
                            <div className="flex flex-col gap-4 items-center justify-center text-center h-[25vh]">
                                ...
                                <h3 className="text-2xl font-bold">Theres no Research & Knowledge</h3>
                            </div>
                        )}
                    </div>
                </div>
