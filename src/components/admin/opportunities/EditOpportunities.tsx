...
const EditOpportunity = ({ location, params }: { location: any; params: any }) => {
   ...
   const [opportunityType, setOpportunityType] = useState("");
        ...
        const fetchOpportunity = async () => {
            ...
            try {
                ...
                setFormData({
                    ...
                    title_fr: opportunity.title_fr ?? "",
                    description_en: opportunity.description_en ?? "",
                    description_fr: opportunity.description_fr ?? "",
                    location_en: opportunity.location_en ?? "",
                    location_fr: opportunity.location_fr ?? "",
                    status: opportunity.status ?? "",
                    type: opportunity.type ?? "",
                    contract_type: opportunity.contract_type ?? "",
                    due_date: opportunity.due_date ?? "",
                    postStart: opportunity.postStart ?? "",

    
                });

                // Parse any JSON content
                const parsedContentItems = opportunity.content_items.map((item: any) => {
                    if (item.type === "list") {
                ...
                if (!["en", "fr"].includes(paramLang || "")) {
                    setLanguage(opportunity.title_en ? "en" : "fr");
                }
            } catch (error) {
                console.error("Error fetching blog opportunity:", error);
            }
        };
    ...
    const handleItemContentChange = (itemIndex: number, opportunity: any) => {
        ...
        if (item.type === "image" || item.type === "pdf") {
            const file = opportunity.target.files?.[0];
            if (file) {
                item.file = file;
            ...
            // text, title, etc.
            item.content = opportunity.target.value;
            if (language === "en") {
                setEnglishItems([...updatedItems]);
            } else {
                setFrenshItems([...updatedItems]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await axios.post(`/api/opportunities/${slug}`, formDataToSend, {
