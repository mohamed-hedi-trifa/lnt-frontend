...
const EditTraining = ({ location, params }: { location: any; params: any }) => {
  ...
  useEffect(() => {
    const fetchtraining = async () => {
      ...
      try {
        ...
        const training: any = response.data;
        // Sort content items by order
        training.content_items.sort((a: any, b: any) => a.order - b.order);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`/api/training/${slug}`, formDataToSend, {
      ...
      <form onSubmit={handleSubmit} className="flex flex-col grow">
        ...
        <div className="mb-4">
          ...
          <div className="mt-2 flex space-x-4">
            <label
              className={`flex items-center p-2 border rounded cursor-pointer duration-200 ${formData.type === "formation" ? "bg-indigo-100 hover:bg-indigo-200" : "bg-white hover:bg-slate-100"
                }`}
            >
              <input
                id="formation"
