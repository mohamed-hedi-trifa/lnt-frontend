...
const UpdateNews = ({ location, params }: { location: any; params: any }) => {
    ...
    useEffect(() => {
        const fetchNews = async () => {
            ...
            try {
                ...
                const news: any = response.data;


                news.content_items.sort((a: any, b: any) => a.order - b.order);
                ...
                setFormData({
                    ...
                    description_fr: news.description_fr ?? "",
                    card_description_en: news.card_description_en ?? "",
                    card_description_fr: news.card_description_fr ?? "",
                    date: news.date ?? "",
                    status: news.status ?? "",
                ...
                const parsedContentItems = news.content_items.map((item: any) => {
                    if (item.type === "list") {
                ...
                if (!["en", "fr"].includes(paramLang || "")) {
                    setLanguage(news.title_en ? "en" : "fr");
                }
            } catch (error) {
    ...
    const handleLanguageChange = (e: React.ChangeNews<HTMLSelectElement>) => {
        setLanguage(e.target.value);
    };

    const handleChange = (e: React.ChangeNews<HTMLInputElement | HTMLTextAreaElement>) => {
        const { name, value } = e.target;
        setFormData((prev) => ({ ...prev, [name]: value }));
    };

    const handleImageChange = (e: React.ChangeNews<HTMLInputElement>) => {
        if (e.target.files?.[0]) {
            setImage(e.target.files[0]);
        }
    };

    const handleItemContentChange = (itemIndex: number, news: any) => {
        ...
        if (item.type === "image" || item.type === "pdf") {
            ...
            // text, title, etc.
            item.content = news.target.value;
            if (language === "en") {
                setEnglishItems([...updatedItems]);
            } else {
                setFrenshItems([...updatedItems]);
    ...
    const handleSubmit = async (e: React.FormNews) => {
        e.preventDefault();
        ...
        try {
            const response = await axios.post(`/api/news/${slug}`, formDataToSend, {
