...
const CreateNews: React.FC = () => {
    ...
    const handleItemContentChange = (itemIndex: number, news: any) => {
        ...
        if (updatedItems[itemIndex].type === "image" || updatedItems[itemIndex].type === "pdf") {
            const file = news?.target?.files?.[0]; // Get the selected file
            ...
            updatedItems[itemIndex].content = news.target.value;
            language === "en" ? setEnglishItems([...updatedItems]) : setFrenchItems([...updatedItems]);
        }
    };
    ...
    const validateForm = () => {
        ...
        if (language === "en") {
            ...
            if (!formData.description_fr) {
                newErrors.summary_fr = "description is required.";
            }
        ...
        if (!formData.date) {
            newErrors.date = "News type is required.";
        }
        setErrors(newErrors);
        return Object.keys(newErrors).length === 0;
    };

    async function createNews(): Promise<{ slug: string } | null> {
        if (!validateForm()) {
            return null;
        }
