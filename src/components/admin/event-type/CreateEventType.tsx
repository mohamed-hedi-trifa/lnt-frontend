...
const CreateEventType: React.FC = () => {
    ...
    const handleLanguageChange = async (e: ChangeEvent<HTMLSelectElement>) => {
        ...
        if (
            (lang === "en" && (!formData.name_en)) ||
            (lang === "fr" && (!formData.name_fr   ))
        ) {
...
// @ts-ignore
    ...
    async function createPreviousEvent(): Promise<{ slug: string } | null> {
        ...
        setIsLoading(true);
        try {
            const response = await axios.post("/api/event-type", formDataToSend, {
                headers: {
        ...
        <div className="h-[calc(100vh-80px)] flex flex-col p-4">
            <div className="flex justify-between">
                <div className="flex items-center gap-1">
                    <Link className="" to="/admin/event-type">
                        <ArrowLeftIcon className="h-5 w-5" />
                    </Link>
                    <Title>Create New Event Type</Title>
                </div>
