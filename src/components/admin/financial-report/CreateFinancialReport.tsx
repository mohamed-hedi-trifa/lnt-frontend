...
const CreateFinancialReport: React.FC = () => {
    ...
    const handleLanguageChange = async (e: ChangeEvent<HTMLSelectElement>) => {
        ...
        if (result.isConfirmed) {
            ...
            if (created) navigate(`/admin/financial-report/${created.slug}?lang=${lang}`);
    ...
    async function createFinancialReport(): Promise<{ slug: string } | null> {
        if (!validateForm()) {
            return null;
        }
        ...
        setIsLoading(true);
        try {
            const response = await axios.post("/api/financial-report", formDataToSend, {
                headers: {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (created) navigate("/admin/financial-report");
    };
    ...
    return (
        <div className="h-[calc(100vh-80px)] flex flex-col p-4">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col grow">
                ...
                <Input label="Date" type="date" name="date" value={formData.name} onChange={handleChange} />
