...
export default function FinancialSource() {
  ...
  const deleteItem = (e: any, item: any) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`/api/financial-source/${item.id}`)
          .then((res) => {
      ...
      <div className="max-w-[80rem] p-2 sm:p-5 mx-auto">
        <div className="rounded-lg shadow-lg">
          <div className="flex justify-between items-center bg-gray-100 p-3 rounded-t-lg">
            <h5 className="mb-3">Financial Sources ( {itemsList.length} )</h5>
            <Link to="/admin/financial-source/create" className="bg-blue-600 text-white p-2 rounded">
              Create Financial Sources
            </Link>
          </div>
          <div className="p-5">
            {itemsList?.length > 0 ? (
              <>
                <div className="mx-0 grid grid-cols-12 text-center break-all">
                  ...
                  <div className="pb-3 text-start col-span-2">Period</div>
                  <div className="pb-3 text-start col-span-2">Source</div>
                  <div className="pb-3 text-start col-span-2">Titre</div>
                  <div className="pb-3 text-start col-span-2">Description</div>
                  <div className="pb-3 text-start col-span-2">Amount</div>
                  ...
                  {itemsList?.map((item: any) => {
                    return (
                      <div key={item.id} className="mx-0 grid grid-cols-12 text-center break-all">
                        ...
                        <div className="pt-3 col-span-2 text-start">{item.period}</div>
                        <div className="pt-3 col-span-2 text-start">{item.source}</div>
                        <div className="pt-3 col-span-2 text-start">{item.title_en || item.title_fr}</div>
                        <div className="pt-3 col-span-2 text-start">{item.description_en || item.description_fr}</div>
                  
                       
                        <div className="pt-3 col-span-2 text-start">{item.amount}</div>
              ...
              <div className="flex flex-col gap-4 items-center justify-center text-center h-[25vh]">
                ...
                <h3 className="text-2xl font-bold">Theres no Financial Sources</h3>
              </div>
            )}
          </div>
        </div>
