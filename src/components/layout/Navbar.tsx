...
function Navbar({ location }: { location: any }) {
  ...
  // Récupération des types d'événements via API et mise à jour du menu
  useEffect(() => {
    const fetchEventTypes = async () => {
      try {
        ...
        // Transformation des types d'événements pour le menu
        ...
        // Mise à jour de la section "Événements" avec les événements dynamiques
        const updatedItems = [
          {
  ...
  // Fonction d'aide pour normaliser les chemins (supprimer le slash final)
  const normalizePath = (path: string) => path.replace(/\/$/, "");

  // Fonction d'aide pour vérifier si un item (ou l'un de ses sous-items) est actif
