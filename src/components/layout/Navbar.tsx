...
function Navbar({ location }: { location: any }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await axios.get("/api/event-type/visible");
        const transformedEventTypes = data.map((event: any) => ({
        ...
        setItems([
          ...
          { label: "Opportunités", path: "/opportunities", underlineClassName: "w-[90px]" },
  ...
  const isActive = (item: any): boolean => {
    ...
    if (item.path && (current === normalize(item.path) || current.startsWith(normalize(item.path) + "/")))
      return true;
    return item.items?.some((sub: any) => isActive(sub));
  };
      ...
      >
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          ...
          <div className="grow flex gap-3 items-center justify-end">
            <NavbarSearch />
            <div className="hidden md:inline h-8 w-px bg-white" />
            <LangLink
              to="/benevole"
              className="hidden md:inline relative px-4 py-2 text-white text-sm font-semibold rounded-full shadow-[-1px_2px_5px_rgb(0,0,0,.3)] bg-[linear-gradient(to_right,_#50ACC6,_#00E676,_#50ACC6)] bg-[length:200%_100%] bg-left hover:bg-right transition-all duration-300"
            >
              DEVENIR BÉNÉVOLE
            </LangLink>
                        ...
                        <div className="relative group">
                          ...
                          {subItem.path ? (
                            subItem.dynamic ? (
                              <Link
                                to={subItem.path}
                                className="block py-2.5 px-2 font-semibold text-black hover:bg-gradient-to-r from-[#0887BECC] to-[#4FACC5CC] transition-colors duration-300 nav-dropdown-item"
                                style={{ textShadow: "0px 4px 4px rgb(0,0,0,.4)" }}
                              >
                                {subItem.label}
                              </Link>
                    ...
                    >
                      {(open) => (
                        ...
                        >
                          ...
                          />
                          <div
                            className={`${item.underlineClassName || "w-[100px]"} absolute bottom-[-6px] h-[4px] rounded-lg
                            bg-gradient-to-r from-[#3344DC] to-[#50ACC6] transform
...
function ArrowDown({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 14 7" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
