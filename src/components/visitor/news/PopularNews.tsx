...
export default function PopularNews() {
    const newsData = [
        { image: recentArticle1, category: "Formation", title: "Formation sur les fondamentaux de la gestion des aires marines protégées (MPA) organisée par MedPAN en Turquie", date: "LE 4 OCTOBRE 2024" },
        { image: recentArticle1, category: "Environnement", title: "Nouvelle réglementation pour la protection des récifs coralliens", date: "LE 5 OCTOBRE 2024" },
        { image: recentArticle1, category: "Technologie", title: "Une startup tunisienne développe un drone marin autonome", date: "LE 6 OCTOBRE 2024" },
        { image: recentArticle1, category: "Énergie renouvelable", title: "Un projet solaire innovant voit le jour en Tunisie", date: "LE 7 OCTOBRE 2024" },
        { image: recentArticle1, category: "Santé", title: "Découverte d'un remède naturel contre certaines maladies de peau", date: "LE 8 OCTOBRE 2024" },
    ];
                    ...
                    {newsData.slice(0, 2).map((news, index) => (
                        <div className="relative text-start w-full">
                            ...
                            <div className="absolute left-[30px] bottom-[20px] right-[60px]  text-white flex flex-col gap-1 sm:gap-4">
                                <button type="button" className="rounded-[3px] bg-white/30 text-white font-semibold py-2 w-fit px-3 text-sm">
                                    {news.category}
                                </button>
                    ...
                    {newsData.slice(2).map((news, index) => (
                        <div className="relative text-start w-full">
                            ...
                            <div className="absolute left-[20px] bottom-[10px] text-white flex flex-col gap-1">
                                <button type="button" className="rounded-[3px] bg-white/30 text-white font-semibold w-fit px-2 text-xs py-1">
                                    {news.category}
                                </button>
    ...
    overflow-hidden text-ellipsis whitespace-normal break-words max-w-full">
            ...
            <div className='block mt-10 sm:hidden'>
                <div className="flex justify-between items-center gap-3">
                    ...
                    <PopularNewsCard image={newsData[currentIndex].image} category={newsData[currentIndex].category} title={newsData[currentIndex].title} date={newsData[currentIndex].date} />
                ...
                <div className='flex w-full justify-center mt-5 gap-3'>
                    {newsData.map((_, index) => (
                        <div key={index} className={`rounded-full w-3 h-3 ${index === currentIndex ? 'bg-[#0270A0]' : 'bg-black/30'}`}></div>
                    ))}
                </div>
            </div>
        </div>
