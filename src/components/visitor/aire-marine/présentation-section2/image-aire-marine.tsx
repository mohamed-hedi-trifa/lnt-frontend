...
const ImageAireMarine: React.FC<ImageAireMarineProps> = ({ title, description, imageUrl }) => {
    return (
        <div
            className=" w-full max-w-[616px] w-[372px] h-[227px] md:h-[320px] relative bg-cover rounded-[15px] flex items-center justify-center transition-all duration-500 hover:brightness-75 group"
        ...
        >
            <div className="text-white flex flex-col md:flex items-start justify-center gap-4 sm:gap-6 px-4 sm:px-6 relative">
                <div className="transition-transform duration-300 group-hover:-translate-x-2 group-hover:-translate-y-5 flex flex-col gap-6">
                    <p className="font-bold text-[20px] sm:text-[25px] md:text-[30px] transition-all">
                        {title}
                    </p>
                    <p className="text-sm font-semibold text-[18px] md:text-[20px] leading-[20px] sm:leading-[28px] md:leading-[32px] transition-all">
                        {description}
                    </p>
                </div>
                <div className="images">
                    <div
                        className="  justify-center  overflow-hidden absolute fixed top-16 md:top-20  right-4  bottom-20 md:right-10  w-[100px] h-[0px] group-hover:h-[100px] duration-300"
                        style={{
