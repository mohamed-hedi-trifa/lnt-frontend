...
const images = [
  { title: 'Explorer le suivi scientifique', description: 'Protégeons ensemble la biodiversité de kerkennah', imageUrl: '/images/aire_marines/presentation/img6.jpg', path: '/protected-air-marine-coastal-areas/monitoring' },
  { title: 'Revivez Nos Expériences Scientifiques', description: 'Découvrez les formations et campements déjà organisés', imageUrl: '/images/aire_marines/marine4.jpg', path: '/protected-air-marine-coastal-areas/training/' },
]
        ...
        <section className="w-full flex flex-col sm:flex-row relative sm:gap-8 sm:py-10 pb-10">
          ...
          <section className="w-fit flex flex-col gap-8 mt-5 sm:mt-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {loading ? Array.from({ length: 6 }).map((_, i) => <TeamCardSkeleton key={i} />) : teamMembers.map(m => <TeamCard key={m.id || m.name} member={m} />)}
            </div>
          </section>
        </section>
