...
export default function PopularEventType3({ events, eventTypeTitle, language = "fr" }: { events: any, language: string , eventTypeTitle: string}) {
                    ...
                    <React.Fragment key={index}>
                        ...
                        <Link to={`/event/event-details/${event.slug}`}  className="flex flex-col gap-2 ">
                            <img src={`${process.env.GATSBY_API_URL}${event?.image}`}
                                className=' object-cover aspect-square rounded-xl sm:w-[301px] sm:h-[253px] h-[258px]'
