...
const ButtonCard: React.FC<ButtonProps> = ({
    ...
    return (
        <button
            className={`${bg ? bg : ""} ${variant ? VARIANTS[variant] : "bg-primary hover:bg-primaryHover text-white font-semibold py-2 px-4 rounded-full focus:outline-none focus:ring focus:ring-blue-300"} ${customClassnames}`}
            {...rest}
        >
            {children}
        </button>
